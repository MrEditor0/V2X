...
warning_dict={'EBW':101,'UFCW':102,'FOW':103,'PCW':104,'ICW':105,'RCW':106,'FCW':107,'VRUCW':108,'BSW':109,'LCW':110,
    ...
    'EVW':121,'STBSD':122,'RLVW':123,'HLW':124,'SLW':125,'CSWS':126,'AVW':127
}
#veh_name 主车的名字
veh_name = "Veh_1"
...
def output(time):
    ...
    for sensor in sensorList:
        if sensor.available():
            ...
            json_obj = json.loads(bsm)
            #print(dir(sensor))
            gps_thread(host_id)
            can_thread(host_id)
            v2x_thread(json_obj[0],host_id)
...
def v2x_thread(bsm_message,sensor_owner):
